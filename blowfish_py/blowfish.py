#!/usr/bin/env python
# blowfish.py
# Copyright (C) 2020 Peng Lian <penglian518@foxmail.com>
#
...
class Blowfish:
    ...
    def __init__(self, key):
        ...
        for i in range(len(self.p_boxes)):
            val = (int.from_bytes((key[index % key_len]).encode(), 'big') << 24) + \
                  (int.from_bytes((key[(index + 1) % key_len]).encode(), 'big') << 16) + \
                  (int.from_bytes((key[(index + 2) % key_len]).encode(), 'big') << 8) + \
                  (int.from_bytes((key[(index + 3) % key_len]).encode(), 'big'))
...
class FileEncrypt:
    ...
    def split_line(self, s, n=8):
        """
        :param s: a long string
        :param n: lenght of the chunk
        :return: a list of splited chunks
        """
        s_list = [s[i:i + n] for i in range(0, len(s), n)]
        s_list[-1] = s_list[-1] + '$' * (n - len(s_list[-1]))
        return s_list

    def split_encrypted_line(self, s, n=8):
        """
        :param s: a long encrypted string
        :param n: lenght of the chunk
        :return: a list of splited chunks
        """
        s_list = [s[i:i + n] for i in range(0, len(s), n)]
    ...
    def encrypt_line(self, s):
        ...
        s_list = self.split_line(s)
        s_encrypted = ''.join([cipher.encrypt(i) for i in s_list])
        return s_encrypted

    def decrypt_line(self, s):
        """
        :param s: a long encrypted string
        ...
        s_list = self.split_encrypted_line(s)
        s_encrypted_list = [cipher.decrypt(i) for i in s_list]
        s_encrypted_list[-1] = s_encrypted_list[-1].strip('$')
        s_decrypted = ''.join(s_encrypted_list)
        return s_decrypted
    ...
    def Encrypt(self, inFile, outFile):
        start = time.time()
        # read the file
        fcon = open(inFile).readlines()
        print("Total lines: ", len(fcon))
        # encrypt
        fcon_encrypted = [self.encrypt_line(i) for i in fcon]
        # write
        pickle.dump(fcon_encrypted, open(outFile, 'wb'))

        end = time.time()
        print('The encryption time is: %.2f' % (end - start))

    def Decrypt(self, inFile, outFile):
        start = time.time()
        # read the file
        #fcon = open(inFile).readlines()
        fcon = pickle.load(open(inFile, 'rb'))
        print("Total lines: ", len(fcon))
        # decrypt
        fcon_decrypted = [self.decrypt_line(i) for i in fcon]
        # write
        open(outFile, 'w').write(''.join(fcon_decrypted))
        end = time.time()

        print('The decryption time is: %.2f' % (end - start))


class pFileEncrypt(FileEncrypt):
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()  # id of processor
    size = comm.Get_size()  # total number of the processors

    def split_a_list(self, alist, n):
        ...
        l = len(alist)
        Inputt = iter(alist)
        length_to_split = [l // n + 1] * (l % n) + [l // n] * (n - (l % n))
    ...
    def Encrypt(self, inFile, outFile):
        ...
        if self.rank == 0:
            ...
            data_split = self.split_a_list(data, self.size)
        ...
        data = self.comm.scatter(data_split, root=0)
        #print('rank', self.rank, 'has data:', data)
        # update the data with your own function
        data = [self.encrypt_line(i) for i in data]
        # gather the calculated data
        newData = self.comm.gather(data, root=0)

        # show the result
        if self.rank == 0:
            # flatten the nested list
            fcon_encrypted = [i for j in newData for i in j]
    ...
    def Decrypt(self, inFile, outFile):
        ...
        # update the data with your own function
        data = [self.decrypt_line(i) for i in data]
        ...
        if self.rank == 0:
            # flatten the nested list
            fcon_decrypted = [i for j in newData for i in j]
...
if __name__ == '__main__':
    def test():
        ...
        print(len(text), len(crypted), len(decrypted))
